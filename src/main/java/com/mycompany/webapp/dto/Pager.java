...
public class Pager {
	...
	public Pager(int rowsPerPage, int pagesPerGroup, int totalRows, int pageNo) {
		...
		totalPageNo = totalRows / rowsPerPage; //전체 행 수/한페이지에 들어가는 행 수 
		if(totalRows % rowsPerPage != 0) totalPageNo++; //만약 나누어 떨어지지 않고 나머지가 남으면 1페이지 추가
		
		totalGroupNo = totalPageNo / pagesPerGroup; //전체 페이지 수/그룹당 페이지 수
		if(totalPageNo % pagesPerGroup != 0) totalGroupNo++; // 딱 떨어지지 않으면 +1
		
		groupNo = (pageNo - 1) / pagesPerGroup + 1; // 현재 페이지를 알 때, 현재페이지 -1 / 그룹당 페이지 +1
		
		startPageNo = (groupNo-1) * pagesPerGroup + 1; //시작 페이지는 그룹 번호 기준으로 정한다.
		
		endPageNo = startPageNo + pagesPerGroup - 1; // 마지막은 이게 성립하지 않는다.
